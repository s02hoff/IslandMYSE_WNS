...
cap$NN <- cap$Net_events * cap$Nets
cap$NNlog <- log10(cap$NN)

cap_sum <- cap %>%
  group_by(Location)%>%
  summarise(sumMYSE=sum(MYSE),sumNN=sum(NN), n=n())

cap.glm1 <- glm(Cap ~ YSW * Loc2 + offset(NNlog), data=cap, family = "binomial")
summary(cap.glm1)
cap_simresid1 <- simulateResiduals(cap.glm1)
plot(cap_simresid1)
plot_model(cap.glm1, type = "pred", terms = c("YSW[0:10]","Location"))
